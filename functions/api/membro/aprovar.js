export async function onRequestPost(context) {
  try {
    ...
    // Se vier uma lista de IDs ou for para aprovar todos os pendentes
    else {
      if (ids && Array.isArray(ids) && ids.length > 0) {
        // Aprovar IDs específicos
        ...
        // Aprovar todos os pendentes (caso ids seja vazio/null)
        await context.env.ARVORE_FAMILIA_DB.prepare(`
          UPDATE membros 
          SET status = 'approved' 
